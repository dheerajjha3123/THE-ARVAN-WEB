...
export async function orderProcessed(
  ...
  try {
    // Validate and fallback empty parameters
    const safeName = name && name.trim() !== "" ? name.trim() : "User";
    const safeItem = item && item.trim() !== "" ? item.trim() : "product";
    const safeRegards = regards && regards.trim() !== "" ? regards.trim() : "Regards";
        ...
        components: [
          {
            ...
            parameters: [
              ...
              { type: "text", text: safeItem },
              { type: "text", text: safeRegards },
            ],
          },
        ],
      },
